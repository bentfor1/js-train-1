...
function calculateSum(n) {
  ...
  for (
    let i = 1;
    i <= n;
    i++ // Запустити цикл для змінної i від 1 до n
  ) {
    sum += i;
...
function isPrimeNumber(num) {
  if (num <= 1) {
    return false;
  } // Якщо число num менше або дорівнює 1, то воно не є простим, тому повертаємо false // В іншому випадку продовжуємо перевірку
  else {
    for (
      let i = 2;
      i <= num - 1;
      i++ // Запускаємо цикл для змінної i від 2 до num-1
    )
      if (num % i === 0) {
        return false;
...
function makeAdder(x) {
  return function getAdder(
    y // Вертаємо нову функцію, яка приймає `y`
  ) {
    return x + y;
...
function memoizedArea() {
  let prevSide = null;
  let prevResult = null; // Ініціалізуємо примітивні змінні для зберігання попереднього аргументу та результату
  return function calculateArea(
    side // Повертаємо функцію яка приймає аргумент side - сторону квадрата
  ) {
    if (side === prevSide) {
...
function compose(func1, func2) {
  return function (
    n // Повертаємо нову функцію, яка приймає аргемент n
  ) {
